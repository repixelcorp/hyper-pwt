...
const buildWebCommand =
  async (
    ...
    try {
      ...
      const newTypingsFilePath =
        path.join(
          typingsPath,
          `${widgetName}Props.d.ts`,
        );
      const typingContents =
        await generateTypesFromFile(
          originWidgetXmlPath,
          "web",
        );
      ...
      const customViteConfigPath =
        path.join(
          PROJECT_DIRECTORY,
          VITE_CONFIGURATION_FILENAME,
        );
      const viteConfigIsExists =
        await pathIsExists(
          customViteConfigPath,
        );
      ...
      if (
        ...
        const userConfig =
          await getViteUserConfiguration(
            customViteConfigPath,
          );

        resultViteConfig =
          await getViteDefaultConfig(
            false,
            userConfig,
          );
      } else {
      ...
      const originPackageXmlPath =
        path.join(
          PROJECT_DIRECTORY,
          "src/package.xml",
        );
      const destPackageXmlPath =
        path.join(
          WEB_OUTPUT_DIRECTORY,
          "package.xml",
        );
      const destWidgetXmlPath =
        path.join(
          WEB_OUTPUT_DIRECTORY,
          `${widgetName}.xml`,
        );
      ...
      const editorConfigViteConfig =
        await getEditorConfigDefaultConfig(
          isProduction,
        );
      const editorPreviewViteConfig =
        await getEditorPreviewDefaultConfig(
          isProduction,
        );
      ...
      const mpkFileDestPath =
        path.join(
          outputDir,
          mpkFileName,
        );
      const mendixWidgetDirectory =
        await getMendixWidgetDirectory();
      const mendixMpkFileDestPath =
        path.join(
          mendixWidgetDirectory,
          mpkFileName,
        );
