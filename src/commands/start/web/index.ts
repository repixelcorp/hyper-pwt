...
const startWebCommand =
  async () => {
    try {
      ...
      const distDir =
        await getViteWatchOutputDirectory();
      const distIsExists =
        await pathIsExists(
          distDir,
        );
      const hotReloadWidgetPath =
        path.join(
          distDir,
          `${widgetName}.mjs`,
        );
      ...
      showMessage(
        `${COLOR_GREEN("Widget hot reload is ready!")}`,
      );
      showMessage(
