...
export function parseWidgetXML(
  xmlContent: string,
): WidgetDefinition {
  const parser =
    new XMLParser(
  ...
  if (
    ...
    throw new Error(
      "Invalid widget XML: missing widget element",
    );
  }
  ...
  const widgetDef: WidgetDefinition =
    {
      ...
      pluginWidget:
        ...
        "true",
      offlineCapable:
        widget.offlineCapable ===
        "true",
      supportedPlatform:
        (widget.supportedPlatform as
          | "All"
          | "Native"
          | "Web") ||
        "Web",
...
function parseProperty(
  ...
  if (
    prop.associationTypes
  ) {
    property.associationTypes =
      parseAssociationTypes(
        prop.associationTypes,
      );
  }
...
function parseSystemProperty(
  sysProp: ParsedXMLSystemProperty,
): SystemProperty {
